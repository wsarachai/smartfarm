...
- `water-temp-node/` — **PlatformIO / STM32Cube HAL** firmware for a **NUCLEO-WL55JC1** (STM32WL55JC, `board = nucleo_wl55jc`, `framework = stm32cube`, flash via onboard/external **ST-Link**). A **battery-powered LoRa** hot/cold **water-temperature** sensor node — the repo's **first non-ESP, first LoRa, first sub-GHz** device. Every `WAKE_INTERVAL_S` (default 15 min) it wakes from **Stop2** (RTC/LSE), powers **2× DS18B20** (one per GPIO, SKIP-ROM; hot=PA10, cold=PA9) through an **A0341 P-MOSFET** high-side gate (PA8, active-low, whole rail incl. pull-ups; 100k gate pull-up = OFF while Hi-Z), reads both temps (12-bit, hand **bit-banged** with a DWT µs delay), measures the battery via **VREFINT** (no external parts; assumes battery *is* the 3.0–3.6 V rail), TX's **one uplink** (AS923 **923.2 MHz SF9BW125 CR4/5 14 dBm**), then sleeps. **Uplink-only** (never listens). It can't reach the HTTP web-server directly, so a **12-byte binary frame** (`src/lora/lora_packet.h`: magic/nodeId/seq/flags/temp_hot·temp_cold int16 centi-°C/battery_mv/CRC-8) goes to **`lora-gateway/`**. The LoRa driver (`src/lora/subghz_lora.c`) is a lean **self-contained SX126x command driver over ST's `HAL_SUBGHZ`** — deliberately **not** the full CubeWL SubGHz_Phy middleware; RF-switch truth table (FE_CTRL1=PC4, FE_CTRL2=PC5, FE_CTRL3=PC3) + TCXO-on-DIO3 taken verbatim from ST's NUCLEO-WL55JC BSP. `include/stm32wlxx_hal_conf.h` is project-local (enables `HAL_SUBGHZ`, off in ST's default). No `secrets.h` (raw LoRa needs no keys); config in committed `include/node_config.h`. Debug log on LPUART1 (PA2/PA3 → ST-LINK VCP).
- `lora-gateway/` — **PlatformIO / STM32Cube HAL** firmware for a **second NUCLEO-WL55JC1** — the **RX-only counterpart to `water-temp-node`**. Listens continuously on the shared AS923 channel (shares the node's `src/lora/*` + `src/board.*` + `include/stm32wlxx_hal_conf.h` as **byte-identical copies** — change both together), validates CRC-8, maps `node_id → device_id` (`include/gateway_config.h`; unknown → `water-node-<id>`), and prints **one JSON line per frame** out LPUART1/VCP (adding `rssi`/`snr`/`seq`; invalid temps omitted; `#`-prefixed diagnostic lines). LED2 (PB9) blinks per RX. Its host companion **`lora-gateway/bridge/`** is a standalone **Node.js + `serialport`** forwarder (kept out of `web-server/` to keep it hardware-agnostic, like the AI poller): reads the gateway's `/dev/ttyACM0` (or `COMx`), validates, and POSTs each line to `/api/v1/telemetry` (`SERVER_URL`); auto-reconnects; needs Node ≥ 18 (global `fetch`); `.env` via `--env-file`. So the full path is **node (LoRa) → gateway (WL55, RX) → bridge.js (USB→HTTP) → web-server:3000**, no LoRaWAN/gateway-vendor stack. Dashboard shows `water-temp-01` with `temp_hot`/`temp_cold`/`battery_v`/`rssi`/`snr`.

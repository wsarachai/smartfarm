...
2026-... # node 1: frame v3, air temp/humidity from SHT45
2026-... -> water-temp-01 {"temp_hot": 23.75, "temp_cold": 23.87, "air_temp": 24.13, "humidity": 58.2, "pressure": 1013.2, "co2": 812, "rssi": -41, "snr": 9.2, "seq": 5}
```
...
`lora_packet.py` decodes **all three** — no receiver change is needed to keep older nodes working. Each version only appends, so a field never moves and `receiver.py` emits a metric only when the frame's flag bit for it is set.
...
### Testing the decoder
`test_lora_packet.py` checks byte-compatibility against vectors generated by compiling the firmware's own `lora_packet.h` on the host, covering v1/v2/v3, negative temperatures, the invalid-probe sentinel, and every single-byte corruption. It is stdlib-only — no Pi and no radio needed:
...
- `lora_packet.py` — decode of the v1/v2/v3 frame; port of `lora_packet.h` (kept byte-compatible).

...
| **U6, U7** | TI **LM5164DDAR** | 100 V, 1 A synchronous buck, ~10 µA I_q. **One part number for both rails** — only `R_FB2`, `R_RON` and `R_A` differ (5.1 V vs 3.3 V); the full schematic and every value is in `hardware-interface.md` §5 *The bucks themselves*. Alternates rated ≥60 V: **LM5160ADNTR**, **LMR51440** — both are different enough that the ripple-injection values have to be recalculated, not copied. Avoid the popular 28 V parts (TPS54202, MP1584): a 24 V bank hits 28.8 V on absorb and you have no margin |

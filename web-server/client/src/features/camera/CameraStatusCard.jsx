...
export default function CameraStatusCard() {
  ...
  const age = ageMs == null ? '—' : t('common.secondsAgo', { n: Math.round(ageMs / 1000) });
    ...
    <WidgetCard>
      <div className="p-5 flex flex-col justify-between h-full min-h-[230px]">
        ...
        <div className="mt-4 flex justify-end">
          <Link
            to="/cameras"
            title={t('camera.openLive')}
            className="border border-secondary text-secondary px-4 py-2 font-label-caps text-label-caps font-bold hover:bg-secondary/10 transition-colors active:scale-95 text-center w-full sm:w-auto"
          >
            {t('camera.view')}
          </Link>
        </div>

...
import { Camera, Maximize2, ExternalLink, Users, Radio } from 'lucide-react';
...
const SNAPSHOT_URL = '/api/v1/camera/frame.jpg';
...
// --- Live viewport ----------------------------------------------------------
function LiveViewport({ status, label, ageMs, clients, viewportRef }) {
  const [imgError, setImgError] = useState(false);
  const hasFrame = status !== 'offline';
    ...
    >
      ...
      <div className="absolute top-4 left-4 flex flex-col gap-1">
        <div className="bg-black/40 backdrop-blur-md px-2 py-1 border-l-2 border-primary flex items-center gap-2">
          ...
          <span className="font-data-mono text-label-caps text-white">{label}</span>
        </div>
        <div className="bg-black/40 backdrop-blur-md px-2 py-1 border-l-2 border-secondary">
          <span className="font-data-mono text-label-caps text-white">CAM_ID: ESP32-CAM</span>
        </div>
      </div>
      <div className="absolute top-4 right-4 bg-black/40 backdrop-blur-md px-2 py-1 flex items-center gap-2">
        <Users size={14} className="text-white" />
        <span className="font-data-mono text-label-caps text-white">{clients ?? 0} · {ageLabel(ageMs)}</span>
      </div>
    </div>
  );
}
...
function NodeMetrics({ status, ageMs, bytes, clients }) {
  const rows = [
    { k: 'Status', v: status.toUpperCase(), cls: status === 'online' ? 'text-primary' : status === 'stale' ? 'text-tertiary' : 'text-error' },
    { k: 'Last Frame', v: ageLabel(ageMs), cls: 'text-on-surface' },
    { k: 'Frame Size', v: bytes ? `${(bytes / 1024).toFixed(0)} KB` : '—', cls: 'text-on-surface' },
    { k: 'Viewers', v: String(clients ?? 0), cls: 'text-on-surface' },
  ];
  return (
    <div className="panel rounded-lg p-5 border-t-2 border-t-secondary">
      <h3 className="font-headline-sm text-headline-sm text-on-surface mb-4">Node Metrics</h3>
      <div className="space-y-4">
        {rows.map((r, i) => (
          <div key={r.k} className={`flex justify-between items-center ${i < rows.length - 1 ? 'border-b border-outline-variant/40 pb-2' : ''}`}>
            <span className="font-label-caps text-label-caps text-on-surface-variant uppercase">{r.k}</span>
            <span className={`font-data-mono text-data-mono ${r.cls}`}>{r.v}</span>
          </div>
        ))}
      </div>
  ...
  return (
    <div className="panel rounded-lg p-5">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-headline-sm text-headline-sm text-on-surface">Recent Captures</h3>
        <span className="font-label-caps text-label-caps text-on-surface-variant">SESSION</span>
      </div>
      {captures.length === 0 ? (
        <p className="font-data-mono text-xs text-on-surface-variant">No captures yet — hit “Capture Image”.</p>
      ) : (
        <div className="grid grid-cols-2 gap-2">
          {captures.map((c) => (
            ...
            >
              <img src={c.url} alt={`capture ${c.time}`} className="object-cover w-full h-full grayscale group-hover:grayscale-0 transition-all" />
              <div className="absolute bottom-1 left-1 bg-black/60 px-1">
                <span className="font-data-mono text-[8px] text-white">{c.time}</span>
              </div>
            </a>
          ))}
        </div>
...
function SensorChips({ devices }) {
  ...
  for (const d of devices.filter((x) => x.type !== 'actuator')) {
    for (const [key, value] of Object.entries(d.metrics || {})) {
      if (typeof value === 'number') chips.push({ id: `${d.device_id}::${key}`, key, value });
    }
  }
  ...
  return (
    <div className="space-y-2">
      {chips.slice(0, 4).map((c, i) => {
        ...
        return (
          <div key={c.id} className="bg-surface-container-low border border-outline-variant p-3 flex items-center gap-3">
            <div className={`w-1 h-8 ${colors[i % colors.length]}`} />
            <div className="min-w-0">
              <p className="font-label-caps text-label-caps text-on-surface-variant truncate">{label}</p>
              <p className="font-data-mono text-data-mono text-on-surface">
                {formatMetricValue(c.value)}{unit ? ` ${unit}` : ''}
              </p>
            </div>
          </div>
        );
...
export default function CamerasPage() {
  ...
  const { online, hasFrame, ageMs, bytes, clients } = useSelector(selectCameraStatus);
  const devices = useSelector(selectAllDevices);

  const status = !hasFrame ? 'offline' : online ? 'online' : 'stale';
  const label = status === 'online' ? 'LIVE' : status === 'stale' ? 'STALE' : 'OFFLINE';
  ...
  useEffect(() => () => capturesRef.current.forEach((c) => URL.revokeObjectURL(c.url)), []);
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4 bg-surface-container-low p-3 border border-outline-variant rounded-lg">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="font-data-mono text-data-mono text-primary">ESP32-CAM</span>
          </div>
        </div>
        <div className="hidden md:flex items-center gap-6">
          <div className="text-right">
            <span className="font-label-caps text-label-caps text-on-surface-variant block uppercase">Frame Size</span>
            <span className="font-data-mono text-data-mono text-on-surface">{bytes ? `${(bytes / 1024).toFixed(0)} KB` : '—'}</span>
          </div>
          <div className="text-right">
            <span className="font-label-caps text-label-caps text-on-surface-variant block uppercase">Viewers</span>
            <span className="font-data-mono text-data-mono text-on-surface">{clients ?? 0}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter">
        ...
        <div className="lg:col-span-9 space-y-gutter">
          <div className="relative">
            <LiveViewport status={status} label={label} ageMs={ageMs} clients={clients} viewportRef={viewportRef} />
            <button
              type="button"
        ...
        <div className="lg:col-span-3 space-y-gutter">
          <NodeMetrics status={status} ageMs={ageMs} bytes={bytes} clients={clients} />

...
| Connects cleanly, works fine, then drops into a **fast permanent reconnect loop the moment a command is sent** (Step 11) | The hub policy is missing `iot:Publish` on `.../shadow/update` — it can receive the command but can't report the outcome back, and AWS IoT Core disconnects the client on that unauthorized publish attempt. Since the failed report leaves `desired != reported`, the same delta gets redelivered on every reconnect, so it loops forever. Fixed in the current `HubIotPolicy` (see `aws-cloud-bridge.md`); if you provisioned before this fix, redeploy `smartfarm-cloud` (`ampx sandbox`) to pick up the corrected policy, or bump it live: `aws iot create-policy-version --policy-name smartfarm-hub-policy --policy-document file://<corrected policy json> --set-as-default` |
